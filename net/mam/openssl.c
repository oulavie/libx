...
bool ssl_init( void )
{
  ...
  case secure_protocol_tlsv1_1:
    logprintf( LOG_NOTQUIET,
               _( "Your OpenSSL version is too old to support TLSv1.1\n" ) );
    goto error;

  case secure_protocol_tlsv1_2:
    logprintf( LOG_NOTQUIET,
               _( "Your OpenSSL version is too old to support TLSv1.2\n" ) );
    goto error;
#endif
  ...
  default:
    logprintf(
        LOG_NOTQUIET,
        _( "OpenSSL: unimplemented 'secure-protocol' option value %d\n" ),
        opt.secure_protocol );
    logprintf( LOG_NOTQUIET,
               _( "Please report this issue to bug-wget@gnu.org\n" ) );
    abort();
  }
  ...
  if ( opt.secure_protocol == secure_protocol_pfs )
    SSL_CTX_set_cipher_list(
        ssl_ctx, "HIGH:MEDIUM:!RC4:!SRP:!PSK:!RSA:!aNULL@STRENGTH" );
  ...
  if ( opt.cert_file )
    if ( SSL_CTX_use_certificate_file(
             ssl_ctx, opt.cert_file, key_type_to_ssl_type( opt.cert_type ) ) !=
         1 )
      goto error;
  if ( opt.private_key )
    if ( SSL_CTX_use_PrivateKey_file(
             ssl_ctx, opt.private_key,
...
#define FD_TO_SOCKET( X ) ( X )
  ...
  if ( !ctx->sess )
    logprintf( LOG_NOTQUIET,
               "WARNING: Could not save SSL session data for socket %d\n", fd );
...
static bool pkp_pin_peer_pubkey( X509 *cert, const char *pinnedpubkey )
{
  ...
  len2 =
      i2d_X509_PUBKEY( X509_get_X509_PUBKEY( cert ), (unsigned char **)&temp );
...
bool ssl_check_certificate( int fd, const char *host )
{
  ...
  if ( !pinsuccess )
  {
    logprintf( LOG_ALWAYS,
               _( "The public key does not match pinned public key!\n" ) );
    success = false;
  }
...
To connect to %s insecurely, use `--no-check-certificate'.\n" ),
  ...
  return !pinsuccess ? false
                     : ( opt.check_cert == CHECK_CERT_ON ? success : true );
}
